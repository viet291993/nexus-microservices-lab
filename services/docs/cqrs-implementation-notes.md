...
2. Cấu hình Elasticsearch trong docker-compose:
   - **Dev:** `xpack.security.enabled=true`, `xpack.security.http.ssl.enabled=false`, `discovery.type=single-node`
   - **Production:** `xpack.security.http.ssl.enabled=true` và cấu hình certificate; `discovery.type=single-node` chỉ dùng cho dev/test
